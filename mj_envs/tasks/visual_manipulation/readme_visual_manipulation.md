...
- A variable/adaptive beta via one softened-weight solve (command 90, let the optimizer relax toward reachable) -- 2026-07-10, does not work with cuRobo: cuRobo's trajopt is a local optimizer that deforms a joint-space guess out of obstacles, so a "softer weight" only widens its basin by widening the deformation envelope rather than steering it toward a different posture, and a posture far from the start never gets reached by softening. If an adaptive tilt is wanted, the correct shape is a DESCENDING-beta fallback ladder (try 90, step down until IK-feasible; one-shot, ~+50 ms/grasp), not a solver weight trick.
